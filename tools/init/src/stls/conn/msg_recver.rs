...
impl TcpPackParser {
    ...
    pub fn parse_one_data(&mut self) -> Result<Vec<u8>, ConnErr> {
        if self.buf.len() < LENGH_WIDTH {
            return Err(ConnErr::NoEnoughMsg);
        }

        // parse data_len to avoid tcp pack sticking
        let raw_data = &mut (&self.buf as &[u8]);
        let data_len = read_usize_be(raw_data);
    ...
    pub fn fetch_one_data(&mut self) -> Vec<u8> {
        loop {  // avoid Tcp pack truncation
            match self.parse_one_data() {
                ...
                Err(err) => {
                    match err {
                        ConnErr::NoEnoughMsg => {self.recv_msg_selfconn();},
